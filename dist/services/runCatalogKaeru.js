...
const extractCatalogKaeruData = (text) => {
    ...
    // データのクリーニング処理
    if (result.age)
        result.age = result.age.replace("歳", "").trim();
    if (result.zip)
        result.zip = result.zip.replace("〒", "").trim();
...
const runCatalogKaeru = async (id, pass) => {
...
exports.runCatalogKaeru = runCatalogKaeru;
