export const extractLetter = (medium: string, text: string, object: string, target: string, base: string) => {
    if (!text) return null;
    switch (medium) {
        case 'KH会員登録': {
            ...
            if (object === 'address') {
                ...
                if (zipMatch && zipMatch[0]) return `${value.replace(zipMatch[0], '').trim()}${numberValue}`;
        ...
        case '土地新着ネット': {
            ...
            if (object === 'address') {
                const zipMatch = value.match(/〒\s*([0-9]{3}-?[0-9]{4})?/);
                if (zipMatch && zipMatch[0]) return value.replace(zipMatch[0], '').trim();
                return value;
            }
            return value;
        }
