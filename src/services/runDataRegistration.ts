...
export const runDataRegistration = async (registerData: any, brand: string, pg_mail: string, pg_pass: string) => {
    let pg_id;
    ...
    const fillForm = async () => {
        ...
        for (const item of nameObject) {
            await safeFill(page, registerObject, errors, item.path, item.value, item.label);
        }
        ...
        for (const item of selectObject) {
            await safeSelect(page, registerObject, errors, item.path, item.value, item.label, item.labelPath);
        }
        ...
        if (registerData.zip) {
            ...
            if (zipValue.length === 7) {
                await safeFill(page, registerObject, errors, selectors.zipInput, registerData.zip, 'zip');
        ...
        if (registerData.street) {
            ...
            await safeFill(page, registerObject, errors, selectors.streetInput, streetValue, 'street');
        }
        ...
        if (registerData.building) {
            ...
            await safeFill(page, registerObject, errors, selectors.buildingInput, buildingValue, 'building');
        }
        ...
        try {
            ...
            await safeFill(page, registerObject, errors, '#calendar_item_0_start_at', formattedDate, 'date');
            await page.click(
        ...
        if (registerData.survey && registerData.survey !== '') {
            try {
                ...
                await safeFill(page, registerObject, errors,
