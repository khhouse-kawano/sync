...
import { pgLogin, safeFill, safeSelect, safeGetValue } from '../utils/function';
const errors: string[] = [];
import nodemailer from 'nodemailer';

export const runDataUpdateBeforeInterview = async (updateData: any, brand: string, pg_mail: string, pg_pass: string) => {
    ...
    const fillForm = async () => {
        ...
        for (const item of selectObject) {
            await safeSelect(page, updateObject, errors, item.path, item.value, item.label, item.labelPath);
        }
        ...
        try {
            ...
            if (mobileValue.charAt(0) === '0') {
                await safeFill(
                    page, updateObject, errors, '#customer_customer_contacts_attributes_0_mobile_phone_number',
                    String(mobileValue),
                    'mobile'
                );
            }
            ...
            await safeFill(
                page, updateObject, errors, '#customer_customer_contacts_attributes_0_email',
        ...
        if (updateData.zip) {
            ...
            if (zipValue.length === 7) {
                await safeFill(page, updateObject, errors, selectors.zipInput, updateData.zip, 'zip');
                await page.click(selectors.zipSearchBtn);
                await page.waitForTimeout(1500);

                updateObject.zipContent = await safeGetValue(page, errors,
                    selectors.zipContent,
                    'zip', 'inputValue', ''
                );
                updateObject.prefContent = await safeGetValue(page, errors,
                    selectors.prefContent,
                    'pref',
                    'getAttribute',
                    'value'
                );
                updateObject.cityContent = await safeGetValue(page, errors,
                    selectors.cityContent,
                    'city',
                    'getAttribute',
                    'value'
                );
                updateObject.townContent = await safeGetValue(page, errors,
        ...
        const prefValue = await safeGetValue(page, errors,
            selectors.prefContent,
            'pref',
            'getAttribute',
            'value'
        );
        const cityValue = await safeGetValue(page, errors,
            selectors.cityContent,
            'city',
            'getAttribute',
            'value'
        );
        const townValue = await safeGetValue(page, errors,
            selectors.townContent,
            'town',
            'getAttribute',
            'value'
        ...
        if (updateData.address) {
            ...
            await safeFill(page, updateObject, errors, selectors.streetInput, streetValue, 'street');
        }
        ...
        try {
            ...
            await safeFill(page, updateObject, errors, '#calendar_item_0_start_at', formattedDate, 'date');
            await page.click(
        ...
        for (const item of fillObject) {
            safeFill(page, updateObject, errors, item.path, item.value, item.label);
        }

        // 現居契約形態
        if (updateData.situation) {
            await safeSelect(page, updateObject, errors,
                '#current-contract-type-select',
                updateData.situation,
                'situation',
                'xpath=/html/body/main/div[1]/div[2]/div/form/div[1]/div[10]/div[2]/div[2]/div/div[1]/input'
            );
        }
        ...
        if (updateData.employment.name) {
            try {
                ...
                await safeFill(page, updateObject, errors,
        ...
        const baseXPath =
            'xpath=/html/body/main/div[1]/div[2]/div/form/div[1]/div[7]/div[1]/div[2]/div/div';

        const hasAnyValue = (m: any) => m?.attribute || m?.name || m?.kana || m?.birth;
        ...
        if (updateData.survey && updateData.survey !== '') {
            try {
                ...
                const current = await page.inputValue(
                    'xpath=/html/body/main/div[1]/div[2]/div/form/div[1]/div[4]/div[3]/div[2]/div/div[2]/div[2]/div[1]/textarea'
                );
