...
app.post("/", async (req, res) => {
    ...
    if (registerData.request && registerData.request === 'before_interview'){
        process.nextTick(() => runDataRegistrationBeforeInterview(registerData, brand, pg_mail, pg_pass));
    } else {
...
const runDataUpdateBeforeInterview = async (updateData, brand, pg_mail, pg_pass) => {
    ...
    const fillForm = async () => {
        ...
        const dd = String(today.getDate()).padStart(2, '0');
        const formattedDate = `${yyyy}-${mm}-${dd}`;
        await page.fill('//html/body/main/div[1]/div[2]/div/form/div[1]/div[16]/div[1]/div/turbo-frame/div/div[2]/div[2]/div[1]/div[3]/div[2]/input', formattedDate);
        try{
            updateObject.reserveContent = await page.locator('//html/body/main/div[1]/div[2]/div/form/div[1]/div[16]/div[1]/div/turbo-frame/div/div[2]/div[2]/div[1]/div[3]/div[2]/input').inputValue();
            } catch(e){
            console.warn('入力値失敗:',e);
